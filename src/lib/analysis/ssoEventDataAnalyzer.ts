...
export class SsoEventDataAnalyzer {
    ...
    public analyze(ssoEventDataLog: AnalyzableLog): Analysis {
        ...
        marks.forEach((mark: string) => {
            ...
            if (colonIndex !== -1) {
                ...
                const actionTaken = this.transformSsoEventDataEntry(action, result);
                analysis.appendExplanation(actionTaken);
            } else if (mark) {
                const actionTaken = this.transformSsoEventDataEntry(mark);
                analysis.appendExplanation(actionTaken);
            }
        });
    ...
    public transformSsoEventDataEntry(action: string, result?: string): string {
        ...
        switch (actionString) {
            ...
            case "req_ui_known":
                return `The error code '${this.parseErrorCode(result)}' was emitted as part of the acquire token call. ` +
